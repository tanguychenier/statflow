...
/**
 * POST /api/v1/analytics/{site_id}/aggregate (openapi.yaml `getAggregateMetrics`).
 */
#[Route(
    '/api/v1/analytics/{site_id}/aggregate',
    name: 'analytics_aggregate',
    methods: ['POST'],
)]
final readonly class AggregateMetricsController
{
    public function __construct(
        private AggregateMetricsHandler $handler,
    ...
    public function __invoke(Request $request, string $site_id): JsonResponse
    {
        try {
            ...
            $query = $this->factory->analyticsQuery($siteId, $body);
