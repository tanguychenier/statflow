...
final class InvalidTeamRoleException extends IdentityException
{
    ...
    {
        $message = sprintf(
            '"%s" is not a valid team role. Allowed: %s.',
            $value,
            implode(', ', $allowed),
        );

        return new self($message, [new FieldError('role', 'invalid_enum_value', $message)]);
    }
}
