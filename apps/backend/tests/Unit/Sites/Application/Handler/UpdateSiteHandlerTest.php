<?php

declare(strict_types=1);

/*
 * This file is part of Statflow.
 *
 * (c) Tanguy Chénier <tanguychenier@gmail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace App\Tests\Unit\Sites\Application\Handler;

use App\Shared\Domain\ValueObject\Uuid;
use App\Sites\Application\Command\UpdateSiteCommand;
use App\Sites\Application\Handler\UpdateSiteHandler;
use App\Sites\Domain\Exception\DuplicateSiteDomainException;
use App\Sites\Domain\Exception\PermissionDeniedException;
use App\Sites\Domain\Exception\SiteNotFoundException;
use App\Sites\Domain\Model\Site;
use App\Sites\Domain\Model\TeamRole;
use App\Sites\Domain\Service\SiteAccessPolicy;
use App\Sites\Domain\ValueObject\Hostname;
use App\Sites\Domain\ValueObject\SiteName;
use App\Sites\Domain\ValueObject\Timezone;
use App\Sites\Domain\ValueObject\TrackerKey;
use App\Tests\Unit\Sites\Fake\FrozenClock;
use App\Tests\Unit\Sites\Fake\InMemorySiteRepository;
use App\Tests\Unit\Sites\Fake\InMemoryTeamMembershipProvider;
use DateTimeImmutable;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;

#[CoversClass(UpdateSiteHandler::class)]
final class UpdateSiteHandlerTest extends TestCase
{
    private const USER = '11111111-1111-4111-8111-111111111111';

    private const TEAM = '22222222-2222-4222-8222-222222222222';

    private InMemorySiteRepository $sites;

    private InMemoryTeamMembershipProvider $memberships;

    private Site $site;

    protected function setUp(): void
    {
        $this->sites = new InMemorySiteRepository();
        $this->memberships = new InMemoryTeamMembershipProvider();
        $this->site = Site::register(
            id: Uuid::generate(),
            teamId: Uuid::fromString(self::TEAM),
            name: SiteName::fromString('Old'),
            domain: Hostname::fromString('old.example.com'),
            timezone: Timezone::default(),
            trackerKey: TrackerKey::fromString('stk_' . str_repeat('a', 32)),
            now: new DateTimeImmutable('2026-01-01'),
        );
        $this->sites->save($this->site);
    }

    #[Test]
    public function adminCanUpdateAllFields(): void
    {
        $this->memberships->grant(self::USER, self::TEAM, TeamRole::Admin);

        $view = ($this->handler())(new UpdateSiteCommand(
            actingUserId: self::USER,
            siteId: $this->site->id()->getValue(),
            name: 'New',
            domain: 'new.example.com',
            timezone: 'Europe/Paris',
        ));

        self::assertSame('New', $view->name);
        self::assertSame('new.example.com', $view->domain);
        self::assertSame('Europe/Paris', $view->timezone);
    }

    #[Test]
    public function nullFieldsLeaveValuesUnchanged(): void
    {
        $this->memberships->grant(self::USER, self::TEAM, TeamRole::Admin);

        $view = ($this->handler())(new UpdateSiteCommand(self::USER, $this->site->id()->getValue(), name: 'Only Name'));

        self::assertSame('Only Name', $view->name);
        self::assertSame('old.example.com', $view->domain);
    }

    #[Test]
    public function editorCannotAdministerUpdate(): void
    {
        $this->memberships->grant(self::USER, self::TEAM, TeamRole::Editor);

        $this->expectException(PermissionDeniedException::class);

        ($this->handler())(new UpdateSiteCommand(self::USER, $this->site->id()->getValue(), name: 'X'));
    }

    #[Test]
    public function missingSiteIsNotFound(): void
    {
        $this->memberships->grant(self::USER, self::TEAM, TeamRole::Admin);

        $this->expectException(SiteNotFoundException::class);

        ($this->handler())(new UpdateSiteCommand(self::USER, Uuid::generate()->getValue(), name: 'X'));
    }

    #[Test]
    public function renamingToExistingDomainConflicts(): void
    {
        $this->memberships->grant(self::USER, self::TEAM, TeamRole::Admin);
        $other = Site::register(
            id: Uuid::generate(),
            teamId: Uuid::fromString(self::TEAM),
            name: SiteName::fromString('Other'),
            domain: Hostname::fromString('taken.example.com'),
            timezone: Timezone::default(),
            trackerKey: TrackerKey::fromString('stk_' . str_repeat('c', 32)),
            now: new DateTimeImmutable('2026-01-01'),
        );
        $this->sites->save($other);

        $this->expectException(DuplicateSiteDomainException::class);

        ($this->handler())(new UpdateSiteCommand(self::USER, $this->site->id()->getValue(), domain: 'taken.example.com'));
    }

    #[Test]
    public function changingToOwnDomainIsAllowed(): void
    {
        $this->memberships->grant(self::USER, self::TEAM, TeamRole::Admin);

        $view = ($this->handler())(new UpdateSiteCommand(self::USER, $this->site->id()->getValue(), domain: 'old.example.com'));

        self::assertSame('old.example.com', $view->domain);
    }

    private function handler(): UpdateSiteHandler
    {
        return new UpdateSiteHandler(
            $this->sites,
            new SiteAccessPolicy($this->memberships),
            new FrozenClock(new DateTimeImmutable('2026-02-01T00:00:00+00:00')),
        );
    }
}
